...
lrs=(5e-6 8e-6 1e-5 2e-5 4e-5 6e-5)
nruns=9

for task data_path in ${(kv)tasks}; do
    for lr in $lrs; do
      for idx in $(seq $nruns); do
        PYTHONPATH=. PREFIX="${PREFIX}" SUFFIX="" nohup \
          python fairseq_cli/hydra_train.py -m --config-dir examples/data2vec/config/multi/text_finetuning \
          --config-name $task hydra/launcher=submitit_slurm +run_config=slurm_1g_aws \
          task.data="$data_path" dataset.num_workers=1 \
          model.model_path="$cp" hydra.sweep.dir="$dir/finetune_sweep2/$task/$lr/run_${idx}" \
          "optimization.lr=[${lr}]" +model=text_wrap common.seed=${idx} &
      done
    done
done

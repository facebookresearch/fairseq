...
def get_grid(args):
    ...
    if continued_pretraining:
        ...
        grid += [
            hyperparam('--restore-file', restore_file)
        ]
        grid += [
    ...
    grid += [
        hyperparam('--multilang-sampling-alpha', 0.7, save_dir_key=lambda val: f'alp{val}'),
    ]
    # Default is complete_doc
    if break_mode == 'complete':
        grid += [
            hyperparam('--sample-break-mode', break_mode, save_dir_key=lambda val: f"bm{val}"),
        ]
    # batch size
    grid += [
    ...
    grid += [
        ...
        hyperparam("--warmup-updates", warmup_updates, save_dir_key=lambda val: f"wrm{val}"),
    ]

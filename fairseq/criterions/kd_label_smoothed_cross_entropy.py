...
class KDLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def get_Tprobs(self, output, teacher=True):
        # returns softmax probabilities generated by teacher/student after optional heating
        predict = output[0]
        predict = predict.view(-1, predict.size(-1))
        if teacher:
            return F.softmax(predict/self.task.teacher_temp, dim=-1)
        else:
            return F.log_softmax(predict/self.task.student_temp, dim=-1)
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, teacher_output=None, distil_strategy="word_and_seq_level"):
        ...
        student_lprobs_T = self.get_Tprobs(net_output, teacher=False)
        teacher_probs_T = self.get_Tprobs(teacher_output, teacher=True)

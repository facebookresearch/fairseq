...
class KDLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def get_teacher_lprobs(self, teacher_output):
        ...
        if self.task.temperature_schedule == 'binary':
            ...
            temperature = torch.ones_like(teacher_predict_max) / self.task.temperature # B*T x 1 
            temperature = temperature.masked_fill(teacher_predict_mask, self.task.temperature) # B*T x 1
            ...
            teacher_predict /= self.task.temperature
        teacher_lprobs_T = F.log_softmax(teacher_predict, dim=-1) # B x T x vocab
        return teacher_lprobs_T

    
    def get_student_lprobs(self, student_output):
        # returns softmax probabilities generated by student after optional heating
        student_predict = student_output[0]
        student_predict = student_predict.view(-1, student_predict.size(-1))
        student_predict /= self.task.temperature
        student_lprobs_T = F.log_softmax(student_predict, dim=-1)
        return student_lprobs_T
    ...
    def forward(self, model, sample, reduce=True, teacher_model=None):
        ...
        logging_output = {
            ...
            'KL_loss': extra['KL_loss'].data if extra.get('KL_loss', None) is not None else 0
        }
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, teacher_output=None, distil_strategy="normal"):
        ...
        lprobs_T = self.get_student_lprobs(net_output)
        teacher_lprobs_T = self.get_teacher_lprobs(teacher_output)
        ...
        if teacher_output is None:
            ...
            loss = (((1 - self.alpha) * golden_loss.sum()) + (self.alpha * KL_loss))
            ...
            words_num = nll_loss.size(0)
            word_rate = self.task.distillation_rate
            teacher_lprobs_T = teacher_lprobs_T[~pad_mask]
            ...
            KL_loss = KL_loss[~pad_mask]
            # get gate
            entropy = -1.0 * (teacher_lprobs_T * torch.log(teacher_lprobs_T)).sum(dim=-1)
            ...
            KL_loss = KL_loss[~pad_mask]
            # get gate
            teacher_golden_predict = teacher_lprobs_T.gather(dim=-1, index=target)
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        metrics.log_scalar(
            'kL_loss', 
            kL_loss_sum / ntokens / math.log(2), 
            ntokens, 
            round=3)
        metrics.log_derived(
            'ppl', 

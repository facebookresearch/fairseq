...
def cross_entropy(teacher_probs, student_lprobs, reduction='mean'):
    ...
    if reduction == 'none':
        ...
        ValueError(f"{reduction} is not a valid option for reduction")
...
class KDLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def get_teacher_probs(self, teacher_output):
        ...
        teacher_predict = teacher_predict.view(-1, teacher_predict.size(-1))
        teacher_predict /= self.task.teacher_temp
        teacher_probs_T = F.softmax(teacher_predict, dim=-1)
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, teacher_output=None, distil_strategy="generic"):
        lprobs = model.get_normalized_probs(net_output, log_probs=True)
        ...
        student_lprobs_T = self.get_student_lprobs(net_output)
        ...
        #     KD_loss = KD_loss[~pad_mask]
        #     # get gate
        #     entropy = -1.0 * (teacher_probs_T * torch.log(teacher_probs_T)).sum(dim=-1).view(-1)
        ...
        #     KD_loss = KD_loss[~pad_mask]
        #     # get gate
        #     teacher_golden_predict = teacher_probs_T.gather(dim=-1, index=target).view(-1)

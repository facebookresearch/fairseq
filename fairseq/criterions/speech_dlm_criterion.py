...
class SpeechDLMCriterionConfig(FairseqDataclass):
    ...
    edge_unit_loss_weight: float = field(
        default=1,
        metadata={
            "help": "The weight of the Edge Unit Prediction Loss"
            '(default: 1)'
        }
    )
...
class SpeechDLMCriterion(FairseqCriterion):
    ...
    There are 3 possible losses depending on the targets of the model:
        ...
        - duration_loss : The duration prediction loss, corresponding to
            'duration' target
    """
    def __init__(self, task, sentence_avg, main_and_cross_weights,
                    general_unit_loss_weight, edge_unit_loss_weight, duration_loss_weight
                    ):
        super().__init__(task)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        for channel in loss_dict:
            for pred_channel in loss_dict[channel]:
                ...
                log_keys = {
                    'next':'general_token', 'edge':'edge_token', 'duration':'edge_duration'}
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        target_prefixes = set([item[:-5].split(']')[-1] for item in logging_keys
                                if item.endswith("_loss")])
        for channel_prefix in channels:
            for target_prefix in target_prefixes:
                prefix = "{}{}".format(channel_prefix, target_prefix)
                count_sum = sum(log.get("{}_count".format(prefix), 0)
                                for log in logging_outputs)
                correct_sum = sum(log.get("{}_correct".format(prefix), 0)
                                  for log in logging_outputs)
                loss_sum = sum(log.get("{}_loss".format(prefix), 0) for
                               log in logging_outputs)
                ...
                if 'duration' not in target_prefix:
                    ...
                    metrics.log_derived(
                        "{}_ppl".format(prefix),
                        lambda meters, prefix=prefix:
                        utils.get_perplexity(meters["{}_loss".format(prefix)].avg)
                    )
                else:
                ...
                metrics.log_scalar(
                    "{}_pred_acc".format(prefix), accuracy, round=3
                )

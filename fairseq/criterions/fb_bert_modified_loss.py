...
@register_criterion('bert_modified_loss')
class BertModifiedLoss(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if net_output[1]['target'] is not None:
            lm_targets = net_output[1]['target'].contiguous().view(-1)
        else:

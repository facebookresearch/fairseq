...
class BlockPairDataset(torch.utils.data.Dataset):
    ...
    def _mask_block(self, sentence, mask_ratio=.15):
        ...
        for i in range(sent_length):
            if i in mask:
                ...
                if rand < 0.8:
                    ...
                    sentence[i] = np.random.choice(self.tokens)
            else:
                target[i] = self.pad
...
class BertDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        ...
        masked_blk1, masked_tgt1 = self._mask_block(self.dataset.tokens[block1[0]:block1[1]], index, mask_ratio=self.mask_ratio)
        masked_blk2, masked_tgt2 = self._mask_block(self.dataset.tokens[block2[0]:block2[1]], index, mask_ratio=self.mask_ratio)

...
def collate(
    ...
    if any(constraints):
        constraints = [constraints[x] for x in sort_order]
...
class ConstrainedDataset(FairseqDataset):
    """
    A torch.util.data.Dataset pairs with target-side constraints.

    Args:
        src (torch.utils.data.Dataset): source dataset to wrap
        src_sizes (List[int]): source sentence lengths
        src_dict (~fairseq.data.Dictionary): source vocabulary
        constraints (???, optional): list of target-side constraints
    ...
    def collater(self, samples):
        ...
        Returns:
            dict: a mini-batch with the following keys:
                ...
                - `constraints` (???): A list of constraint lists
        """
        return collate(

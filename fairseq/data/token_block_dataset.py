...
class TokenBlockDataset(FairseqDataset):
    ...
    def __init__(self, dataset, sizes, block_size, pad, eos, break_mode=None, include_targets=False):
        ...
        if break_mode is None or break_mode == 'none':
            ...
            cumsum = torch.cumsum(torch.from_numpy(sizes), dim=0)
        ...
        if break_mode == 'eos':
            ...
            self.block_to_dataset_index = torch.stack(
                [
                    torch.arange(len(sizes)),  # starting index in dataset
                    torch.zeros(len(sizes), dtype=torch.long),  # starting offset within starting index
                    torch.arange(len(sizes))  # ending index in dataset
                ],
                dim=1,
            )
        else:

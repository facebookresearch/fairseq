...
def transform_eos_collater(
    ...
    """
    Args:
        samples (List[dict]): samples to transform
    ...
    Returns:
        List[dict]: transformed samples
    """
    _check_src = _check_tgt = False

    def check_eos(item, key, expect_eos):
        if _check_src:
            assert (t[-1] == eos[0]) == expect_eos
            _check_t = False  # only check once

    def check_tgt(src, expect_eos):
        if _check_src:
            assert (src[-1] == eos[0]) == expect_eos
            _check_src = False  # only check source once

    def transform(item):
        if append_eos_to_src:
            _check_src(item['source'], expect_eos=False)
            item['source'] = torch.cat([item['source'], self.eos])
        if remove_eos_from_src:
            _check_src(item['source'], expect_eos=True)
            item['source'] = item['source'][:-1]
        if append_eos_to_tgt:
            _check_tgt(item['target'], expect_eos=False)
            item['target'] = torch.cat([item['target'], self.eos])
        if remove_eos_from_tgt:
            _check_tgt(item['target'], expect_eos=True)

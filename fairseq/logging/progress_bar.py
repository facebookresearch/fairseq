...
class MlflowProgressBar(BaseProgressBar):
    ...
    def __init__(self, wrapped_bar):
        ...
        self._save_dir = None
        if mlflow is None:
            logger.warning("mlflow not found, pip install mlflow to start logging to mlflow")
            return
    ...
    def __del__(self):
        ...
        if self.mlflow is not None:
            ...
            if best_model_path.exists():
                logger.info("Mlflow: Logging artifacts and model, this might take some time")
    ...
    def _flatten_params(params_dict, parent_key="", sep="/"):
        ...
        for key, value in params_dict.items():
            ...
            if isinstance(value, MutableMapping):
                items.extend(MlflowProgressBar._flatten_params(value, new_key, sep).items())
            else:
    ...
    def _log_to_mlflow(self, stats, tag=None, step=None):
        ...
        for key in stats.keys() - {"num_updates"}:
            if isinstance(stats[key], AverageMeter):
                self.mlflow.log_metric(key=prefix + key, value=float(stats[key].val), step=step)
            elif isinstance(stats[key], Number):
                self.mlflow.log_metric(key=prefix + key, value=float(stats[key]), step=step)

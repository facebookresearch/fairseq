...
class TransformerEncoderLayerBase(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg, return_fc=False):
        ...
        self.in_proj_weight = torch.nn.Parameter(torch.zeros(self.self_attn.q_proj.weight.shape[0]*3, self.self_attn.q_proj.weight.shape[1]))
        self.in_proj_bias = torch.nn.Parameter(torch.zeros(self.self_attn.q_proj.bias.shape[0]*3))

        if self.activation_fn is torch.nn.functional.relu or isinstance(self.activation_fn, torch.nn.ReLU) or self.activation_fn == "relu":
            self.activation_relu_or_gelu = 1
        elif self.activation_fn is torch.nn.functional.gelu or isinstance(self.activation_fn, torch.nn.GELU) or self.activation_fn == "gelu":
            self.activation_relu_or_gelu = 2
        else:
            self.activation_relu_or_gelu = 0
        # Batch first can not be justified but needs user to make sure
        self.can_use_fastpath = not self.normalize_before and self.activation_relu_or_gelu and (self.self_attn_layer_norm.eps == self.final_layer_norm.eps)
        ...
        if 'fb' in torch.__version__:
            ...
            self.int_version = int(self.torch_version[0])*1000 + int(self.torch_version[1])*10 + int(self.torch_version[2])
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        ...
        new_name = prefix
        state_dict[new_name + "in_proj_weight"] = torch.cat((q_proj_weight, k_proj_weight, v_proj_weight), dim=0)
        state_dict[new_name + "in_proj_bias"] = torch.cat((q_proj_bias, k_proj_bias, v_proj_bias), dim=0)

        super(TransformerEncoderLayerBase, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs)
    ...
    def forward(
        ...
        if self.BT_version and x.dim() == 3 and self.load_to_BT and not self.return_fc and self.can_use_fastpath and not self.training and not self.ever_training and not self.cfg_checkpoint_activations:

...
class BaseLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        assert ddp_backend in {"no_c10d", "legacy_ddp"}, f"Incompatible backend {ddp_backend} for MOE models"
        ...
        self.experts = nn.ModuleList([nn.Sequential(*([BaseSublayer(args) for
                                      _ in range(args.base_sublayers)])) for
                                      _ in range(self.num_local_experts)])
    ...
    def forward(self, input_features, *args, **kwargs):
        ...
        input_splits_merged = self.merge_splits(self.expert_count, self.num_workers, input_splits) \
            if not is_training else None

        output_splits_merged = self.merge_splits(self.expert_count, self.num_workers, output_splits) \
            if not is_training else None

        # Swap these tokens for the right ones for our expert
        routed_features = All2All.apply(features[sort_by_expert], output_splits_merged, input_splits_merged)
        ...
        if self.num_local_experts == 1:
            if routed_features.size(0) > 0:
                ...
                routed_features = alpha * self.experts[0](routed_features) + (1 - alpha) * routed_features
        else:
            start_index = 0
            for index, num_features_to_add in enumerate(input_splits):
                ...
                local_expert_features = routed_features[start_index: start_index + num_features_to_add]

                if local_expert_features.size(0) > 0:
                    alpha = torch.sigmoid(local_expert_features.mv(
                        self.expert_centroids[self.expert_id * self.num_local_experts + local_expert_index])).unsqueeze(1)

                    local_expert_features = alpha * self.experts[local_expert_index](local_expert_features) \
                        + (1 - alpha) * local_expert_features

                    routed_features[start_index: start_index + num_features_to_add] = local_expert_features

                start_index += num_features_to_add

        # Return to original worker and ordering
        result = All2All.apply(routed_features, input_splits_merged, output_splits_merged)[self.inverse_sort(sort_by_expert)]
    ...
    def greedy_assignment(self, scores, k=1):
        ...
        output_splits = torch.zeros((self.expert_count,), dtype=torch.long, device=scores.device)
    ...
    def merge_splits(self, world_size_train, world_size_inference, splits):
        ...
        for i in range(0, world_size_train, local_expert_count):
            splits_stitched[i // local_expert_count] = sum(splits[i: i + local_expert_count])

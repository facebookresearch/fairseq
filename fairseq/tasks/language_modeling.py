...
import time
from fairseq.data.plasma_utils import plasma_store_contains
...
PLASMA_NOT_LAZY = os.getenv('PLASMA_NOT_LAZY', 1)
...
class LanguageModelingTask(LegacyFairseqTask):
    ...
    def load_dataset(
        self, split: str, epoch=1, combine=False, is_master=True, **kwargs
    ) -> MonolingualDataset:
        ...
        lock_path = f"/tmp/{split_path.replace('/', '_')}"
        assert split_path is not None, split_path

        # each process has its own copy of the raw data (likely to be np.memmap)
        ...
        # Master process acquires lock, computes slice indices, writes to plasma and points to plasma
        # Other processes wait for master and then point to plasma
        if is_master or (not self.args.use_plasma_view) or PLASMA_NOT_LAZY:
            # acquire lock, build TokenBlockDataset

            #print(f"master has lock")
            ...
            assert self.args.use_plasma_view
            # Do not allow lazy instantiation of TokenBlockDataset before master writes to plasma
            assert plasma_store_contains(split_path, 2, timeout_ms=30000)  # This blocks
            print(f"Rank > 1 about to make dataset")
            dataset = TokenBlockDataset(
                dataset,
                sizes=None,  # Dont need them, already in plasma

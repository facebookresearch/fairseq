...
def all_reduce(tensor, group=None, op='sum'):
    if is_xla():
        ...
        return xm.all_reduce(op, tensor, groups=group[1])
    else:
        if op == 'sum':
...
def all_gather(tensor_list, tensor, group=None):
    ...
    If *tensor_list* is ``None``, then *group* is required.
    """
    assert tensor_list is not None or group is not None
    if is_xla():
        result = xm.all_gather(tensor, groups=group[1])
        if tensor_list is not None:
            assert result.size(0) == len(tensor_list)
            for i, out_t in enumerate(tensor_list):
                out_t.copy_(result[i])
            return tensor_list
        else:
            world_size = get_world_size(group=group)
            assert result.size(0) == world_size * tensor.size(0)
            return result.chunk(world_size, dim=0)
    else:
...
def initialize_model_parallel(model_parallel_size_, use_xla=False):
    """
    ...
    if rank == 0:
        logging.info(
            'initialized model parallel with size {}'.format(model_parallel_size_)
        )

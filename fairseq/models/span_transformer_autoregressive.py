...
@register_model('span_transformer_autoregressive')
class SpanTransformerAutoregressive(FairseqLanguageModel):
    ...
    def build_model(cls, args, task):
        ...
        decoder = SpanTransformerDecoder(args, task.dictionary, embed_tokens, final_norm=False)
...
@register_model_architecture('span_transformer_autoregressive', 'span_transformer_autoregressive')
...
class SpanTransformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    def forward(self, tokens, segment_labels, enc_mask=None, dec_mask=None, apply_mask=True, mask=None, **unused):
        ...
        if apply_mask or (mask is not None):
            ...
            """if mask is not None:
                doc_mask = segment_labels.eq(0)
                x_dec[doc_mask] = x_enc[doc_mask].clone()"""
        else:
            x_dec = x_enc.clone()
        ...
        if not apply_mask:
            ...
            enc_mask, dec_mask = self.genetate_mask(x_enc, enc_mask, dec_mask)
        
        #assert self._test_mask()
        assert enc_mask.size(0) == bsz or enc_mask.size(0) == 1
        enc_mask = enc_mask[:, :num_words, :num_words]
        dec_mask = dec_mask[:, :num_words, :num_words]
        ...
        return x_dec, {'attn': attn, 'target':None} #, 'inner_states': inner_states}
    ...
    def genetate_mask(self, tensor, enc_mask, dec_mask):
        bsz, dim, _ = tensor.size()
        enc_mask_byte = enc_mask[:, :dim, :dim]
        dec_mask_byte = dec_mask[:, :dim, :dim]
        enc_mask = utils.fill_with_neg_inf(tensor.new(bsz, dim, dim))
        dec_mask = utils.fill_with_neg_inf(tensor.new(bsz, dim, dim))

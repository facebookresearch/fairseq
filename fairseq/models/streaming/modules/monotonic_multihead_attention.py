...
from fairseq.models.streaming.utils.p_choose_strategy import (
    learnable_p_choose,
    waitk_p_choose,
)

from fairseq.models.streaming.utils.monotonic_attention import (
...
from fairseq.models.streaming.utils import register_monotonic_attention
...
class MonotonicAttention(MultiheadAttention):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--load-pretrain-monotonic-decoder",
            type=str,
            metavar="EXPR",
            default=None,
            help=""" path to the pretrained monotonic decoder """,
        )
        # fmt: on
...
class WaitKAttention(MonotonicInfiniteLookbackAttention):
    ...
    def __init__(self, args):
        ...
        del self.k_proj
        del self.q_proj
        self.consecutive_writes = getattr(args, "waitk_consecutive_writes", 1)
        self.set_lagging(args.waitk_lagging)

    def set_lagging(self, lagging: int):
        self.waitk_lagging = lagging
        assert (
    ...
    def p_choose_from_qk(
        ...
        p_choose = waitk_p_choose(
            ...
            key_padding_mask=key_padding_mask,
            consecutive_writes=self.consecutive_writes,

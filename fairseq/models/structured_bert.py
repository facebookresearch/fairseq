...
class BertStructuredSelfAttention(nn.Module):
    def __init__(self, config):
        super(BertStructuredSelfAttention, self).__init__()
    ...
    def forward(self, hidden_states, attention_mask, permutation=None):
        ...
        Args:
            ...
            attention_mask : FloatTensor of shape (Batch, 1, 1, Sentence, Token)
            permutation    : LongTensor of shape (Sentence, ), a bijective mapping about
            which sentence attends to which sentence.
        """
        ...
        # q, k, v are now of shape (Batch, Sentence, Token, Head, Head_Dim)

        if permutation is not None:
            key_layer = key_layer.index_select(dim=1, index=permutation)
            value_layer = value_layer.index_select(dim=1, index=permutation)

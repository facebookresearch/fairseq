...
class BertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        """
        Args:
            x : FloatTensor of shape (Batch, Sentence, Token, Dim)
        """
    ...
    def forward(self, hidden_states, attention_mask, permuteation=None):
        ...
        Args:
            ...
            attention_mask : FloatTensor of shape (Batch, Sentence, 1, 1, Token), so that is
            can be broadcasted to (Batch, Sentence, Head, Token (from), Token (to))
...
class StructuredBert(FairseqLanguageModel):

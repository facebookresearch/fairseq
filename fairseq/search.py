...
class PreviousTranslationBiasedBeamSearch(Search):
    ...
    def apply_bias(self, lprobs, original_batch_idxs, step):
        ...
        previous_outputs = self.prefix_allowed_tokens_fn(
            original_batch_idxs
        )       
        po_len = max([po.shape[0] for po in previous_outputs]) if previous_outputs else 0
        if step < po_len:
            # pad previous translations with 1 based on max length
            previous_outputs = [nn.functional.pad(po, (0, po_len-po.shape[0]), value=1) for po in previous_outputs] 
            # repeat previous translations 'beam size' times (batch x beam)
            previous_outputs = list(itertools.chain.from_iterable(itertools.repeat(x, beam) for x in previous_outputs))
    ...
    def step(
        ...
        lprobs = self.apply_bias(
            lprobs.view(bsz * beam_size, vocab_size),
            original_batch_idxs,
            step
        ).view(bsz, beam_size, vocab_size)

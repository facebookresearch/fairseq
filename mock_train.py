...
def profiling_memory(trainer):
    ...
    opt_memory = model_memory
    for buffer in opt.state_dict()['state'].values():
        for val in buffer.values():
            if type(val) == torch.Tensor and val.is_cuda:
                opt_memory += val.numel() * val.element_size()
                print(val.dtype)

    estimated_activation_memory = torch.cuda.max_memory_allocated()
    print(model_memory, opt_memory, estimated_activation_memory)

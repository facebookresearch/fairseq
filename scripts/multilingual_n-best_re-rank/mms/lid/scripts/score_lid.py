...
if __name__ == "__main__":
    ...
    preds = [x.strip() for x in open(args.expdir + "/predictions.txt", "r").readlines()]
    ...
    with open(args.expdir + "/results.txt", "w") as f, open(args.expdir + "/confusions.txt", "w") as f2:
        f.write("AGGREGATE RESULTS\n")
        f.write("acc: " + '{:.4g}'.format(correct / total) + "\n")
        f.write("topk_acc: " + '{:.4g}'.format(topk_correct / total) + "\n")

        f.write("\nPER LANG RESULTS\n")
        f.write("lang\tacc\ttopk_acc\n")
        for k in lang_total.keys():
            f.write(k + "\t" + \
                '{:.4g}'.format(lang_correct[k] / lang_total[k]) + "\t" + \
                '{:.4g}'.format(lang_topk_correct[k] / lang_total[k]) + "\n")

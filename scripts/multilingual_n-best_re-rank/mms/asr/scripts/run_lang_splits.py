...
if __name__ == "__main__":
    ...
    parser.add_argument('--lm_dir', type=str, default="/checkpoint/yanb/MMS1_models/lm/cclms")
    args = parser.parse_args()

    langs = [d for d in os.listdir(args.dump)]
    print(f"Running ASR on {len(langs)} languages...", file=sys.stderr)
    ...
    for lang in tqdm(langs):
        ...
        if args.use_lm != 0:
            ...
            if lang in char_lms:
                ...
                cmd = f"""
                PYTHONPATH=. PREFIX=INFER HYDRA_FULL_ERROR=1 python examples/speech_recognition/new/infer.py -m --config-dir examples/mms/asr/config/ --config-name infer_common decoding.type=kenlm distributed_training.distributed_world_size=1 decoding.unique_wer_file=true decoding.beam=500 decoding.beamsizetoken=50 task.data={dump} "common_eval.path='{args.model}'" decoding.lexicon="{lexicon_file}" decoding.lmpath="{lm_file}" decoding.results_path={dst} dataset.gen_subset="{lang_code}:test" decoding.lmweight=2.5 decoding.wordscore=0 {args.extra_infer_args}
                """
        else:
            cmd = f"""

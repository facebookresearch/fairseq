...
if __name__ == "__main__":
    ...
    for lang in langs:
        ...
        scores = [x.strip() for x in open(args.exp + "/" + lang + "/hypo.score.reord", "r").readlines()]
        assert len(ids) == len(word_hyps)
        assert len(ids) == len(unit_hyps)
        assert len(ids) == len(scores)
        for id, word_hyp, unit_hyp, s in zip(ids, word_hyps, unit_hyps, scores):
            if id in data:
                print("Duplicate ID found")
                import pdb;pdb.set_trace()
            data[id] = (word_hyp, unit_hyp, s)

    with open(args.exp + "/hypo.word", "w") as f1, open(args.exp + "/hypo.units", "w") as f2, open(args.exp + "/hypo.score", "w") as f3:

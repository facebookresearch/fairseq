...
if __name__ == "__main__":
    ...
    parser.add_argument('--shuffle_fz', type=int, default = 0)
    parser.add_argument('--shuffle_len', type=int, default = 0)
    parser.add_argument('--zero_slid', type=int, default = 0)
    parser.add_argument('--zero_wlid', type=int, default = 0)
    parser.add_argument('--zero_asr', type=int, default = 0)
    parser.add_argument('--zero_lm', type=int, default = 0)
    parser.add_argument('--zero_fz', type=int, default = 0)
    parser.add_argument('--zero_len', type=int, default = 0)
    args = parser.parse_args()

    if args.length_score == 1:
        s_w, w_w, a_w, l_w, fz_w, le_w = eval(open(args.w, "r").read())['weights']
    else:
        s_w, w_w, a_w, l_w, fz_w = eval(open(args.w, "r").read())['weights']
    ...
    if args.shuffle_fz == 1:
        random.shuffle(fa_zs_new)
    ...
    if args.length_score == 1:
        feats = [[s, w, a, l, fz, le] for s,w,a,l,fz,le in zip(slid_new, wlid_new, asr_new, lm_new, fa_zs_new, len_new)]
    else:
        feats = [[s, w, a, l, fz] for s,w,a,l,fz in zip(slid_new, wlid_new, asr_new, lm_new, fa_zs_new)]
    
    if args.length_score == 1:
        weight = [s_w, w_w, a_w, l_w, fz_w, le_w]
    else:
        weight = [s_w, w_w, a_w, l_w, fz_w]

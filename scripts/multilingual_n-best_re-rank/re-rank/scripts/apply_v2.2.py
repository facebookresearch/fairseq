...
if __name__ == "__main__":
    ...
    if args.length_score == 1:
        s_w, se_w, w_w, a_w, l_w, fm_w, fz_w, prisk_w, pref_w, le_w = eval(open(args.w, "r").read())['weights']
    else:
        s_w, se_w, w_w, a_w, l_w, fm_w, fz_w, prisk_w, pref_w = eval(open(args.w, "r").read())['weights']
    ...
    if args.length_score == 1:
        feats = [[s, se, w, a, l, fm, fz, prisk, pref, le] for s,se,w,a,l,fm,fz,prisk,pref,le in zip(slid_new, slid_ext_new, wlid_new, asr_new, lm_new, fa_mms_new, fa_zs_new, pron_risk_new, pron_ref_new, len_new)]
    else:
        feats = [[s, se, w, a, l, fm, fz, prisk, pref] for s,se,w,a,l,fm,fz,prisk,pref in zip(slid_new, slid_ext_new, wlid_new, asr_new, lm_new, fa_mms_new, fa_zs_new, pron_risk_new, pron_ref_new)]
    
    if args.length_score == 1:
        weight = [s_w, se_w, w_w, a_w, l_w, fm_w, fz_w, prisk_w, pref_w, le_w]
    else:
        weight = [s_w, se_w, w_w, a_w, l_w, fm_w, fz_w, prisk_w, pref_w]

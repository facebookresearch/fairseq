...
if __name__ == "__main__":
    ...
    if args.length_score == 1:
        s_w, w_w, a_w, l_w, fm_w, fz_w, prisk_w, pref_w, le_w = eval(open(args.w, "r").read())['weights']
    else:
        s_w, w_w, a_w, l_w, fm_w, fz_w, prisk_w, pref_w = eval(open(args.w, "r").read())['weights']
    ...
    for x in pron_risk:
        score = float(x)
        if score == 0:
            pron_risk_new.append(math.log(0.000000001))
        else:
    ...
    for x in pron_ref:
        score = float(x)
        if score == 0:
            pron_ref_new.append(math.log(0.000000001))
        else:
    ...
    if args.length_score == 1:
        ...
        feats = [[s, w, a, l, fm, fz, prisk, pref] for s,w,a,l,fm,fz,prisk,pref in zip(slid_new, wlid_new, asr_new, lm_new, fa_mms_new, fa_zs_new, pron_risk_new, pron_ref_new)]
    
    if args.length_score == 1:
        weight = [s_w, w_w, a_w, l_w, fm_w, fz_w, prisk_w, pref_w, le_w]
    else:
        weight = [s_w, w_w, a_w, l_w, fm_w, fz_w, prisk_w, pref_w]

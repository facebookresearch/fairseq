# adds mean_conf as a separate feat
...
if __name__ == "__main__":
    ...
    if args.length_score == 1:
        feats = [[s, w, a, l, fm, fz, prisk, pref, mc, le] for s,w,a,l,fm,fz,prisk,pref,mc,le in zip(slid_new, wlid_new, asr_new, lm_new, fa_mms_new, fa_zs_new, pron_risk_new, pron_ref_new, mean_conf, len_new)]
    else:
        feats = [[s, w, a, l, fm, fz, prisk, pref, mc] for s,w,a,l,fm,fz,prisk,pref,mc in zip(slid_new, wlid_new, asr_new, lm_new, fa_mms_new, fa_zs_new, pron_risk_new, pron_ref_new, mean_conf)]
    ...
    for i in range(args.iters):
        ...
        mc_w = (np.random.rand() - 0.5) * a_scale
        if args.length_score == 1:
            le_w = (np.random.rand() -0.5) * 1
            weights.append([s_w, w_w, a_w, l_w, fm_w, fz_w, prisk_w, pref_w, mc_w, le_w])
        else:
            weights.append([s_w, w_w, a_w, l_w, fm_w, fz_w, prisk_w, pref_w, mc_w])

...
if __name__ == "__main__":
    ...
    parser.add_argument('--mean_slid', type=str, default = None)
    ...
    if args.mean_slid is not None:
        mean_slid = [float(x.strip()) for x in open(args.mean_slid, "r").readlines()]
    ...
    for i, x in enumerate(asr):
        if x == "":
            ...
            # if args.asr_length is not None:
                ...
                val = float(x)
            if args.mean_asr is not None:
                if args.clip_conf == 1 and mean_asr[i] < args.clip_conf_thres:
    ...
    for i in range(args.iters):
        ...
        if args.length_score == 1:
            ...
            weights.append([s_w, w_w, a_w, l_w, fz_w, le_w])
        else:
            weights.append([s_w, w_w, a_w, l_w, fz_w])

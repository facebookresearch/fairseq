...
#         execute(f"rm -rf {file}")

# def convert_audio(audio_file_orig, sampling_freq, output_file, log_info=True):
#     execute(f"ffmpeg -y -i {audio_file_orig}  -ar {sampling_freq} -ac 1  {output_file} -loglevel quiet")
#     if log_info:
#         execute(f"soxi {output_file}")

# import fairseq
# class args:

#     nbest = 1
#     lexicon = '/scratch/padentomasello/gslm_demo/pgslm/checkpoints/lexicon_upper.txt'
#     kenlm_model = '/scratch/padentomasello/gslm_demo/pgslm/checkpoints/lm_common_crawl_large_4gram_prun0-0-5_200kvocab.bin'
#     # kenlm_model = '/private/home/padentomasello/code/DSAlign/models/en/lm_librispeech_word_transformer.pt'
#     # beam = 1500
#     # beam_size_token = 100
#     # beam_threshold = 25.0
#     # lm_weight = 2
...
#     def __init__(self, path, decoder_type):
...
#         # print(task.cfg)
#         # overrides = {}
#         # self.model, self.cfg, self.task = fairseq.checkpoint_utils.load_model_ensemble_and_task([path], task=task, arg_overrides=overrides)
#         self.model, self.cfg, self.task = fairseq.checkpoint_utils.load_model_ensemble_and_task([path], arg_overrides=overrides)
#         for model in self.model:
#             # model.make_generation_fast_(beamable_mm_beam_size=500, need_attn=False)
#             model.eval()
#             model.cuda()
#             pass
...
#         w2l_decoder = self.decoder_type
#         if w2l_decoder == "viterbi":
#             from examples.speech_recognition.w2l_decoder import W2lViterbiDecoder

#             return W2lViterbiDecoder(args, self.task.target_dictionary)
#         elif w2l_decoder == "kenlm":
#             from examples.speech_recognition.w2l_decoder import W2lKenLMDecoder

#             return W2lKenLMDecoder(args, self.task.target_dictionary)
#         elif w2l_decoder == "fairseqlm":
#             from examples.speech_recognition.w2l_decoder import W2lFairseqLMDecoder

#             return W2lFairseqLMDecoder(args, self.task.target_dictionary)
#         else:
#             print(
...
#             toks.int().cpu(),
#             self.task.cfg.eval_wer_post_process,
#             escape_unk=True,
#         )
#         return s

#     def __call__(self, **kwargs):
#         return self.model[0].forward(**kwargs)
...
#         print(audio.shape)
#         gen_out = self.task.inference_step(self.generator, self.model , { 'net_input': { 'source' : audio, 'padding_mask': None }}, None)
#         if len(gen_out) == 0:
#             return ''
#         if len(gen_out[0]) == 0:
#             return ''
#         print(gen_out)
#         hyp = self.decode(gen_out[0][0]["tokens"])
#         return hyp.lower()
...
"""
falign_ext = torch.utils.cpp_extension.load_inline("falign", cpp_sources="", cuda_sources=cuda_source, extra_cflags=['-O3', '-DC10_USE_GLOG'], verbose=True )
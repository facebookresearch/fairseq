...
def get_grid(args):
    ...
    return [
        ...
        hyperparam('--arch', ['clas_dualinputs2twavtransformer_base_stack'],), #need small learning rate
        ...
        hyperparam('--criterion', ['label_smoothed_ce_ctxgate_reg', ]),
        hyperparam('--alpha', 0.01, save_dir_key=lambda val: f'alpha{val}'),
        ...
        hyperparam('--context-attention-type', 'sequential', save_dir_key=lambda val: f"ctxtype_{val}"),
        hyperparam('--add-context-gating', save_dir_key=lambda val: f"ctxgate"),
        hyperparam('--context-activation-fn', 'softmax', save_dir_key=lambda val: f"ctxattnfn_{val}"),

...
import logging
from examples.speech_text_joint_to_text.modules.sequence_generator_context import SequenceGeneratorWithContext
from fairseq.data.concat_dataset import ConcatDataset

from fairseq.tasks import register_task

from examples.speech_text_joint_to_text.data.context_wrapper_dataset import SpeechTextContextDataset, SpeechTextFixedContextDataset
...
class SpeechTextContextTask(SpeechTextRetrievalTask):
    ...
    def add_args(cls, parser):
        SpeechTextRetrievalTask.add_args(parser)
        parser.add_argument(
            "--max-positives",
            type=int,
            metavar="N",
            default=3,
            help="number of positives per sample",
        )
...
@register_task("speech_text_context_infer")
class SpeechTextContextInferTask(SpeechTextJointToTextTask):
    def __init__(self, args, src_dict, tgt_dict, infer_tgt_lang_id=None):
        super().__init__(args, src_dict, tgt_dict, infer_tgt_lang_id=infer_tgt_lang_id)
        if SpeechTextContextDataset.CONTEXT_TAG not in tgt_dict:
            tgt_dict.add_symbol(SpeechTextContextDataset.CONTEXT_TAG)
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if isinstance(ds_with_entities, ConcatDataset):
            ...
            ds_with_entities.source_entities = False
        self.datasets[split] = SpeechTextFixedContextDataset(ds_with_entities, self.tgt_dict)
    ...
    def build_generator(
        ...
        return super().build_generator(models, args, seq_gen_cls=SequenceGeneratorWithContext, extra_gen_cls_kwargs=extra_gen_cls_kwargs)

...
if os.getenv("MOSES") is not None:
    ...
    raise TypeError(
        "Setting environement variable MOSESE is required for formatting WMT test files"
    )
...
def download_wat(directory):
    ...
    kmen_download_url = (
        "http://lotus.kuee.kyoto-u.ac.jp/WAT/km-en-data/wat2020.km-en.zip"
    )
...
def download_floresv1(directory):
    ...
    for lang, code_lang in zip(langs, code_langs):
        shutil.move(
            os.path.join(
                tmp_directory, "flores_test_sets", f"wikipedia.devtest.{lang}-en.{lang}"
            ),
            os.path.join(dataset_directory, f"test.eng_Latn-{code_lang}.{code_lang}"),
        )
        shutil.move(
            os.path.join(
                tmp_directory, "flores_test_sets", f"wikipedia.devtest.{lang}-en.en"
            ),
...
def download_wmt(directory):
    ...
    paths = ["wmt13", "wmt14", "wmt16", "wmt17", "wmt18", "wmt19"]  # incorrect order
    ...
    list_rawfiles = [
        ...
        [
            "test.deu_Latn-eng_Latn.deu_Latn",
            "test.deu_Latn-eng_Latn.eng_Latn",
            "test.eng_Latn-fra_Latn.fra_Latn",
            "test.eng_Latn-fra_Latn.eng_Latn",
            "test.eng_Latn-hin_Deva.hin_Deva",
        ...
        [
            "test.ces_Latn-eng_Latn.ces_Latn",
            "test.ces_Latn-eng_Latn.eng_Latn",
            "test.eng_Latn-est_Latn.est_Latn",
            "test.eng_Latn-est_Latn.eng_Latn",
            "test.eng_Latn-tur_Latn.tur_Latn",
            "test.eng_Latn-tur_Latn.eng_Latn",
        ],
        list(
            itertools.chain.from_iterable(
                [
                    [f"test.eng_Latn-{lang}.{lang}", f"test.eng_Latn-{lang}.eng_Latn"]
                    for lang in [
                        "fin_Latn",
                        "guj_Gujr",
                        "kaz_Cyrl",
...
def download_autshumato(directory, pre_download_directory):
    ...
    download_path = os.path.join(
        pre_download_directory, "Autshumato_MT_Evaluation_Set.zip"
    )
    ...
    filename = os.path.join(
        tmp_directory,
        "Autshumato_MT_Evaluation_Set",
        "Autshumato.EvaluationSet.{}.Translator{}.txt",
    )
    for lang, code in zip(langs, codes):
        test_src_out = open(
            "{}/test.eng_Latn-{}.eng_Latn".format(dataset_directory, code), "w"
        )
        test_tgt_out = open(
            "{}/test.eng_Latn-{}.{}".format(dataset_directory, code, code), "w"
        )

        dev_src_out = open(
            "{}/dev.eng_Latn-{}.eng_Latn".format(dataset_directory, code), "w"
        )
        dev_tgt_out = open(
            "{}/dev.eng_Latn-{}.{}".format(dataset_directory, code, code), "w"
        )
...
def download_tico(directory):
    ...
    for lang, code_lang in zip(langs, code_langs):
        ...
        pair = (
            f"eng_Latn-{code_lang}"
            if "eng_Latn" < code_lang
            else f"{code_lang}-eng_Latn"
        )
...
def download_madar(directory, pre_download_directory):
    ...
    tgz_file = os.path.join(
        pre_download_directory, "MADAR.Parallel-Corpora-Public-Version1.1-25MAR2021.zip"
    )
    unzip(tgz_file, tmp_directory)
    madar_dir = os.path.join(
        tmp_directory, "MADAR.Parallel-Corpora-Public-Version1.1-25MAR2021/MADAR_Corpus"
    )
    cities = [
        "Alexandria",
        "Amman",
    ...
    for code in flores:
        for city in flores[code]:
            # City dict:
            src = open(
                os.path.join(dataset_directory, f"test.arb_Arab-{code}.arb_Arab"), "a"
            )
            tgt = open(
                os.path.join(dataset_directory, f"test.arb_Arab-{code}.{code}"), "a"
            )
...
def download_mafand(directory):
    ...
    for tgt in fra_directions:
        ...
        pair = (
            f"fra_Latn-{tgt_nllb_code}"
            if "fra_Latn" < tgt_nllb_code
            else f"{tgt_nllb_code}-fra_Latn"
        )
    ...
    for tgt in eng_directions:
        ...
        pair = (
            f"eng_Latn-{tgt_nllb_code}"
            if "eng_Latn" < tgt_nllb_code
            else f"{tgt_nllb_code}-eng_Latn"
        )

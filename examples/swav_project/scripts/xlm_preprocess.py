...
from fairseq.data.xlm_code_dictionary import XLMDictionary
...
def xlm_build_dictionary(filenames, workers=1, threshold=-1, nwords=-1, padding_factor=8):
    d = XLMDictionary()
    for filename in filenames:
        XLMDictionary.add_file_to_dictionary(
...
def xlm_load_dictionary(filename):
    return XLMDictionary.load(filename)
...
def main(args):
    ...
    def build_dictionary(filenames, src=False, tgt=False):
        assert src ^ tgt
        _dictionary = xlm_build_dictionary(
            filenames,
    ...
    if args.joined_dictionary:
        ...
        if args.srcdict:
            src_dict = xlm_load_dictionary(args.srcdict)
        elif args.tgtdict:
            src_dict = xlm_load_dictionary(args.tgtdict)
        else:
            assert (
                args.trainpref
        ...
        if target:
            if args.tgtdict:
                tgt_dict = xlm_load_dictionary(args.tgtdict)
            else:
                assert (
                    args.trainpref
    ...
    logger.warning(f"Dictionary size: {len(src_dict)}")

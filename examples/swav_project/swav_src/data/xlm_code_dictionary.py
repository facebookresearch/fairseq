...
class XLMDictionary:
    ...
    def __init__(
        ...
        *,  # begin keyword-only arguments
        ...
        # NOTE: reference to the original XLM dictionary implementation,
        self.bos_word, self.unk_word, self.pad_word, self.eos_word = bos, unk, pad, eos
        self.symbols = []
        self.count = []
        self.indices = {}
        self.bos_index = self.add_symbol(bos)
        # NOTE nxphi: switch eos/pad order and add 10 symbles
        self.eos_index = self.add_symbol(eos)
        self.pad_index = self.add_symbol(pad)
        self.unk_index = self.add_symbol(unk)
        self.xlm_special_symbols = [SPECIAL_WORD % i for i in range(SPECIAL_WORDS)]
        self.xlm_special_symbols[1] = "<mask>"
        for s in self.xlm_special_symbols:
            self.add_symbol(s)
        self.mask_word = self.xlm_special_symbols[1]
        self.mask_index = self.index(self.mask_word)

        assert extra_special_symbols is None
    ...
    def add_file_to_dictionary(filename, dict, tokenize, num_workers):
        ...
        if num_workers > 1:
            ...
            for (start_offset, end_offset) in chunks:
                results.append(
                    pool.apply_async(
                        XLMDictionary._add_file_to_dictionary_single_worker,
                        (
                            local_file,
                            tokenize,
                            dict.eos_word,

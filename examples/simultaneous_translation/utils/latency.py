...
class LatencyTraining(object):
    def __init__(
        ...
        stay_on_last_token, average_method="average", var_power=2.0, var_span=1.0,
    ):
        ...
        self.variance_calculator = {
            ...
            "exponential_variance_delay": ExponentialVarianceDelay(var_power, var_span),
            "power_variance_delay": PowerVarianceDelay(var_power, var_span),
            "softmax_variance_delay": SoftmaxVarianceDelay(),
        }
    ...
    def expected_delays_from_attention(
        ...
        """
        attention: 
            (transformer) list of bsz, num_heads, tgt_len, src_len 
            (lstm) bsz, tgt_len, src_len
        source_padding_mask:
            bsz, src_len
        target_padding_mask
            bsz, tgt_len
...
class AverageDelay(LatencyMetric):
    """
    AD = 1 / (|y]) sum_i^|Y| deleys_i
    """
    @staticmethod
    def cal_metric(delays, src_lens, tgt_lens, target_padding_mask):
        AD = torch.sum(delays.masked_fill(target_padding_mask, 0), dim=0, keepdim=True)
        AD = AD / tgt_lens
        return AD
...
class VarianceProportion(LatencyMetricVariance):
    @staticmethod
    def cal_metric(delays, src_lens, tgt_lens, target_padding_mask):
        """
        delays : bsz, num_heads_x_layers, tgt_len
        ...
        target_padding_mask: bsz, tgt_len or None
        """
        props = delays / src_lens.unsqueeze(2)
        variance_props = props.var(dim=1)
        if target_padding_mask is not None:
            variance_props.masked_fill_(target_padding_mask, 0)

        return variance_props.sum(dim=1, keepdim=True) / tgt_lens


class PowerVarianceDelay(LatencyMetricVariance):
    def __init__(self, power, span):
        self.power = power
        self.span = span
    def cal_metric(self, delays, src_lens, tgt_lens, target_padding_mask):
        """
        delays : bsz, num_heads_x_layers, tgt_len
        src_lens : bsz, 1
        target_lens : bsz, 1
        target_padding_mask: bsz, tgt_len or None
        """
        offset_power = torch.pow((delays - delays.mean(dim=1, keepdim=True)) / self.span, self.power)
        variance_delays = torch.sum(offset_power, dim=1) / src_lens
        if target_padding_mask is not None:
            variance_delays.masked_fill_(target_padding_mask, 0)
...
class SoftmaxVarianceDelay(LatencyMetricVariance):
    @staticmethod
    def cal_metric(delays, src_lens, tgt_lens, target_padding_mask, power=2):
        """
        delays : bsz, num_heads_x_layers, tgt_len
        src_lens : bsz, 1
        target_lens : bsz, 1
        target_padding_mask: bsz, tgt_len or None
        """
        offset_power = torch.pow(delays - delays.mean(dim=1, keepdim=True), 2)
        weights = torch.nn.functional.softmax(offset_power, dim=1)
        variance_delays = torch.sum(offset_power * weights, dim=1) / src_lens
        if target_padding_mask is not None:
            variance_delays.masked_fill_(target_padding_mask, 0)
...
class ExponentialVarianceDelay(LatencyMetricVariance):
    def __init__(self, power, span):
        self.power = power
        self.span = span
    ...
    def cal_metric(self, delays, src_lens, tgt_lens, target_padding_mask):
        ...
        target_padding_mask: bsz, tgt_len or None
        """
        offset = (delays - delays.mean(dim=1, keepdim=True)) / self.span
        variance_delays = torch.exp(torch.pow(offset, self.power)).sum(dim=1) / src_lens
        if target_padding_mask is not None:
            variance_delays.masked_fill_(target_padding_mask, 0)

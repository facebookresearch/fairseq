...
output units",
    ...
    parser.add_argument(
        "--w2l-decoder", choices=["viterbi", "kenlm", "ctc_greedy"], help="use a w2l decoder"
    )
...
def main(args):
    ...
    if getattr(args, "kspmodel", None) is not None:

        sp.Load(os.path.join(args.data, args.kspmodel))
    else:
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for sample in t:
            ...
            for i, sample_id in enumerate(sample["id"].tolist()):
                if getattr(task.dataset(args.gen_subset), "speakers", None) is not None:

...
for dataset, version, split in datasets:
    ...
    sd_config = SignDatasetConfig(name="holistic", version=version, include_video=False, include_pose="holistic", extra={'split': split} if split else {})
    ...
    for split in splits:
        ...
        for i, datum in enumerate(data_l):
            ...
            pose_len = datum['pose']['data'].shape[0]

...
def main():
  ...
  parser.add_argument("--source", type=str,
                      help="text file with source sequences", required=True)
  parser.add_argument("--target", type=str,
                      help="text file with target sequences", required=True)
  parser.add_argument("--target-out", type=str,
                      help="where to write target sequences without <unk-N> "
                           "entries",
                      required=True)
  args = parser.parse_args()

  target_in = open(args.target, "r", encoding="utf-8") \
              if args.target is not None else None
  target_out = open(args.target_out, "w", encoding="utf-8") \
               if args.target_out is not None else None
...
try:
  ...
  print("Source sequence length:", len(e.source_seq.strip().split()),
        file=sys.stderr)

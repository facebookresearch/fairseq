...
import sys
import io
from fairseq.data.data_utils import compute_mask_indices, get_bucketed_sizes, get_buckets
...
class FbankDataset(FairseqDataset):
    def __init__(
        self,
        sample_rate,
...
class FileFbankDataset(FbankDataset):
    def __init__(
        self,
        manifest_path,
        sample_rate,
    ...
    def __getitem__(self, index):
        ...
        wav, sr = sf.read(path_or_fp, dtype="float32")
        assert sr == self.sample_rate, sr
        if wav.ndim == 2:
            wav = wav.mean(-1)
        assert wav.ndim == 1, wav.ndim

        #* TODO: add transform of the soundfile
        with torch.no_grad():
            ...
            concat = torch.cat([fbank], dim=0)
            concat = concat.transpose(0, 1).contiguous()  # (freq, time)

        #? normalization needed?
        concat = self.postprocess(concat, curr_sample_rate) # layer_normalization
        return {"id": index, "source": concat}


class BinarizedFbankDataset(FbankDataset):
    def __init__(
        self,
        data_dir,
        split,

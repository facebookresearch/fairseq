    ...
    for src_idx in range(lang_num):
        for tgt_idx in range(src_idx+1, lang_num):
            ...
            src_s2u_manifest = os.path.join(
                manifest_dir, f"{src_lang_code}-{tgt_lang_code}",
                f"test_{domain}.tsv"
            )
            src_asr_manifest = os.path.join(
                manifest_dir, f"{src_lang_code}-{tgt_lang_code}",
                "source_unit", f"test_{domain}.tsv"
            )
            gen_manifest(
                src_aud_manifest,
                src_s2u_manifest,
            ...
            tgt_aud_manifest = os.path.join(
                fleurs_manifest_dir, f"{tgt_lang_code}-{src_lang_code}",
                f"test_{tgt_lang_code}-{src_lang_code}_{tgt_lang_code}.tsv"
            )
            tgt_s2u_manifest = os.path.join(
                manifest_dir, f"{tgt_lang_code}-{src_lang_code}",
                f"test_{domain}.tsv"
            )

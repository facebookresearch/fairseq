...
def extract_and_downsample_segment(
    aud_path, start_time, end_time,
    seg_path, out_sr=16000
):
    if not os.path.exists(seg_path):
        ...
        wav, _ = torchaudio.load(aud_path, frame_offset=start_frame, num_frames=num_frames)
        # downsampled to 16kHz
        ds_wav = F.resample(wav, in_sr, out_sr)
        # save segments
        torchaudio.save(
            seg_path, ds_wav, sample_rate=out_sr
        )
...
def segment_epst_aud(
    epst_dir, src_lang, tgt_lang,
    proc_epst_dir, out_sr=16000, domain="epst"
):
    ...
    out_tsv_dir = os.path.join(proc_epst_dir, "aud_manifests", src_lang+"-"+tgt_lang)
    ...
    for split in ["test"]:
        tsv_fn = os.path.join(
            out_tsv_dir,
            "_".join([split, domain, src_lang, tgt_lang]) + ".tsv"
        )
        ...
        with open(seg_fn, "r") as fin:
            for line in fin:
                ...
                tmp_aud_path = os.path.join(
                    tmp_out_aud_dir, aud + ".wav"
                )
                ...
                nframes = extract_and_downsample_segment(
                    tmp_aud_path, float(st), float(et), seg_path,
                    out_sr=16000
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--epst-dir", type=str, required=True
    )
    parser.add_argument(
        "--proc-epst-dir", type=str, required=True
    )
    parser.add_argument(
    ...
    for src_lang in EP_LANGS:
        for tgt_lang in EP_LANGS:
            ...
            aud_manifest_fn = os.path.join(
                args.proc_epst_dir, "aud_manifests",
                f"{src_lang}-{tgt_lang}",
                f"test_{domain}_{src_lang}_{tgt_lang}.tsv"
            )
            manifest_dir = os.path.join(
                args.save_root, manifest_key,
                f"{src_lang}-{tgt_lang}"
            )
            ...
            normalize_translations(
                args.epst_dir, src_lang, tgt_lang,
                manifest_dir, domain=domain
            )

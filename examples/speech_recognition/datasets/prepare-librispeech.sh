...
fairseq_root=/scratch/cpuhrsch/repos/fairseq
mkdir -p ${out_dir}
cd ${out_dir} || exit

export LD_LIBRARY_PATH=/private/home/cpuhrsch/usr/lib
...
for part in train_960 test-other test-clean; do
  python ${fairseq_root}/examples/speech_recognition/datasets/asr_prep_json.py --num-cpu ${num_cpu} --audio-dirs ${download_dir}/LibriSpeech/${part} --labels ${download_dir}/LibriSpeech/${part}/text --spm-model ${bpemodel}.model --audio-format flac --dictionary ${fairseq_dict} --output ${part}.json
done
# fairseq expects to find train.json and valid.json during training
mv train_960.json train.json

echo "Prepare valid json"
python ${fairseq_root}/examples/speech_recognition/datasets/asr_prep_json.py --num-cpu ${num_cpu} --audio-dirs ${download_dir}/LibriSpeech/dev-clean ${download_dir}/LibriSpeech/dev-other --labels ${download_dir}/LibriSpeech/valid_text --spm-model ${bpemodel}.model --audio-format flac --dictionary ${fairseq_dict} --output valid.json

...
#this code index the phrase generated by stanfordNLP for training .
def indexPhraseData(line, dictionary, append_eos, reverse_order):
...
class Comparable():
    ...
    def sim_matrix(self, a, b, eps=1e-8):
        ...
        return sim_mt, torch.sum(nearestSrc[0], 1), torch.sum(nearestTgt[0], 0)
    ...
    def _get_iterator(self, sent, dictn, max_position, epoch, fix_batches_to_gpus=False):
        ...
        itrs = EpochBatchIterator(dataset=sent, collate_fn=sent.collater, batch_sampler=batch_sampler, seed=1,num_workers=0, epoch=epoch)
            ...
            for article_pair in comp_list:
                ...
                try:
                    if self.representations == 'embed-only':
                        ...
                        it1 = src_article.next_epoch_itr(shuffle=False, fix_batches_to_gpus=False)
                        src_sents += self.get_article_coves(it1, representation='memory', mean=False)
                        ...
                        it3 = tgt_article.next_epoch_itr(shuffle=False, fix_batches_to_gpus=False)
                        tgt_sents += self.get_article_coves(it3, representation='memory', mean=False)

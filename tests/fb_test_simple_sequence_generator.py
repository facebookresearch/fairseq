...
class TestJitSimpleSequeneceGenerator(TestJitSequenceGeneratorBase):
    ...
    def test_ensemble_sequence_generator(self):
        model = self.transformer_model
        generator = ScriptSequenceGenerator([model], self.task.tgt_dict, beam_size=2)
        scripted_model = torch.jit.script(generator)
        self._test_save_and_load(scripted_model)

    def test_generate_with_transformer_model(self):
        model = self.transformer_model
        simple_generator = ScriptSequenceGenerator(
...
class TestSimpleSequeneceGenerator(TestSequenceGeneratorBase):
    ...
    def test_maxlen(self):
        generator = ScriptSequenceGenerator([self.model], self.tgt_dict, beam_size=2, max_len_b=2)
        hypos = generator.generate(self.sample)
        eos, w1, w2 = self.tgt_dict.eos(), self.w1, self.w2
        # sentence 1, beam 1
        self.assertHypoTokens(hypos[0][0], [w1, eos])
    ...
    def test_encoder_with_different_output_len(self):
        ...
        generator = ScriptSequenceGenerator([reshaping_model], self.tgt_dict, beam_size=2, max_len_b=2)
        hypos = generator.generate(self.sample)
        for sent in [0, 1]:
            for beam in [0, 1]:

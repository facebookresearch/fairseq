...
class TestRobertaHub(unittest.TestCase):
    ...
    def test_roberta_large_wsc(self):
        with contextlib.redirect_stdout(StringIO()):
            ...
            ans = roberta.disambiguate_pronoun('The trophy would not fit in the brown _suitcase_ because [it] was too big.')
            self.assertFalse(ans)

            ans = roberta.disambiguate_pronoun('The city councilmen refused the demonstrators a permit because [they] feared violence.')
            self.assertEqual(ans, 'The city councilmen')

            ans = roberta.disambiguate_pronoun('The city councilmen refused the demonstrators a permit because [they] advocated violence.')
